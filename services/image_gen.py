...
def generate_hero_image(prompt):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.warning(
                f"⚠️ Image Endpoint failed ({response.status_code}): {response.text}. Trying Chat Endpoint..."
            )

            # Fallback: Chat Completion Endpoint
            ...
            chat_response = requests.post(
                chat_url, headers=headers, json=chat_payload, timeout=30
            )
            if chat_response.status_code == 200:
                chat_data = chat_response.json()
                logger.info(f"🔍 Full Chat Response: {json.dumps(chat_data)}")
                ...
                try:
                    ...
                    logger.warning(
                        f"⚠️ Failed to extract image from message.images: {e}"
                    )

                logger.warning("⚠️ Could not extract URL from chat response.")
            else:
                logger.error(
                    f"❌ Chat Endpoint also failed: {chat_response.text}"
                )
